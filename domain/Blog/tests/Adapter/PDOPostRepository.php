...
class PDOPostRepository implements PostRepositoryInterface
{
  ...
  {
    $this->pdo = new PDO('mysql:host=localhost;dbname=hexablog_dev;charset=utf8', 'root', '', [
  ...
  public function findOne(string $uuid): ?Post
  {
    ...
    return new Post($result['title'], $result['content'], $result['published_at'] ? new DateTime($result['published_at']) : null, $result['uuid']);
  }
}
